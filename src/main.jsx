...
function App() {
  ...
  const analyze = async (event) => {
    ...
    try {
      ...
      const rawImprovements = Array.isArray(result.improvements) ? result.improvements : [{ category: result.category, scoreChange: result.scoreChange }];
      const improvements = rawImprovements
        .filter((item, index, list) => AI_STATS.includes(item?.category) && list.findIndex((candidate) => candidate?.category === item.category) === index)
        .slice(0, 2)
        .map((item) => ({ category: item.category, scoreChange: Math.max(1, Math.min(3, Math.round(Number(item.scoreChange) || 1))) }));
      if (!improvements.length) improvements.push({ category: job?.stat || AI_STATS[0], scoreChange: 1 });
      const category = improvements[0].category;
      const scoreChange = improvements[0].scoreChange;
      const totalScoreChange = improvements.reduce((total, item) => total + item.scoreChange, 0);
      const stats = { ...state.stats };
      improvements.forEach((item) => { stats[item.category] = Math.min(100, stats[item.category] + item.scoreChange); });
      const today = new Date().toLocaleDateString('sv-SE');
      const continuityChange = state.lastAdventureDate === today ? 0 : 1;
      stats.継続力 = Math.min(100, stats.継続力 + continuityChange);
      const points = state.points + totalScoreChange;
      const nextProgression = getProgress(points);
      const nextJob = getJobForStats(stats);
      const changedJob = nextJob !== state.job;
      const logs = [{ date: new Date().toLocaleDateString('ja-JP'), note, category, scoreChange, totalScoreChange, improvements, continuityChange, tip: result.tip, positive: result.positive, nextAction: result.nextAction, reply: result.reply, progress: result.progress, speaker: result.speaker }, ...state.logs].slice(0, 20);
      ...
      setAnalysis({ ...result, category, scoreChange, totalScoreChange, improvements, nextLevel: nextProgression.level, leveled: nextProgression.level > getProgress(state.points).level, changedJob, nextJob });
      setNote('');
  ...
  return <div className="app-shell">
    ...
    <main>
      {screen === 'jobs' && <section className="screen hero-screen"><div className="battle-scene"><div className="sun" /><div className="pixel-cloud cloud-one" /><div className="pixel-cloud cloud-two" /><div className="mountain" /><div className="dragon-silhouette" /><div className="party-silhouette"><i /><i /><i /></div></div><div className="command-window intro-command"><span className="command-caret">▶</span> 職業一覧</div><p className="lead intro-lead">到達した職業は詳細を確認できます。まだ見ぬ職業はシークレットです。</p><div className="job-grid">{JOBS.map((item) => { const unlocked = Boolean(state.profiles?.[item.id]); return <button className={`job-card job-card-readonly ${unlocked ? '' : 'job-card-secret'}`} style={{ '--accent': unlocked ? item.color : '#687568' }} key={item.id} type="button"><span className="job-icon">{unlocked ? item.icon : '?'}</span>{unlocked ? <><span className="job-name">{item.id}</span><span className="job-catch">{item.catch}</span><span className="choose">到達済み</span></> : <><span className="job-name">？？？</span><span className="job-catch">まだ見ぬ職業</span><span className="choose">SECRET</span></>}</button>; })}</div></section>}
