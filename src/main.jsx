...
function App() {
  ...
  const analyze = async (event) => {
    ...
    try {
      ...
      const logs = [{ date: new Date().toLocaleDateString('ja-JP'), note, category, scoreChange, tip: result.tip, positive: result.positive, nextAction: result.nextAction, speaker: result.speaker }, ...state.logs].slice(0, 20);
